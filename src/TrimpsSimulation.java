...
public class TrimpsSimulation {
    ...
    private final static double expectedDodges = calculateExpectedDodges(
            dodgeLength);

    // average trimp damage per hit
    private final static double damagePerHit = (critDamage * critChance)
            + (1 - critChance);
    ...
    private void staleAvg(double[] res, double wRes, double[] tmp,
        ...
        // hits are reweighted by the likelihood of NOT overkilling the current
        // cell - if we OK we just count zero time
        ...
        if (total > 0) {
            ...
            System.out.format("fresh=%.3f wRes=%.3f wTmp=%.3f" + "%n", freshTmp,
                    wRes, wTmp);
            throw new Error(
